...
public class RawMaterialManagement_AddRawMaterialServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            Pattern patternSKU = Pattern.compile("RM[0-9]+");
            Matcher matchSKU = patternSKU.matcher(SKU);
            
            if (itemManagementBean.checkSKUExists(SKU)) {
                result = "?errMsg=Failed to add raw material, SKU: " + SKU + " already exist.";
                response.sendRedirect(source + result);
            } else if(_length < 1 || width < 1 || height < 1) {
                result = "?errMsg=Value must be greater than or equal to 1.";
                response.sendRedirect(source + result);
            } else if (matchSKU.matches()) {
                ...
                result = "?errMsg=Please match requested format: RM&#35";
                response.sendRedirect(source + result);
            }
